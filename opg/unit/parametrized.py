...
class ParametrizedTestCase(unittest.case.TestCase):
    ...
    def compareRetcodeTest(self):
        ...
        for compareData in compareDataList:
            ...
            if sign:
                ...
                expectData = expectFunData() if (
                    hasattr(expectFunData, '__call__')) else expectFunData
                actualData = self.myservice.compareFuncDict.get(
                    compareData["actualData"])()
                cprFun(
                    expectData, actualData, "testPoint is %s,expectData is %s,actualData is %s" %
    ...
    def assertObject(
        ...
        compareFun = funDict[typeof(
            actualData)] if compareFun is None else compareFun
        compareFun(
            expectData, actualData, "testPoint is %s,expectData is %s,actualData is %s" %
            (testPoint, str(expectData), str(actualData)))

    def id(self):
        return "%s.%s_%s" % (self.__class__.__interfaceName__ +
                             "--" + self.param[0], self.param[0], self.param[2])
# 判断变量类型的函数
